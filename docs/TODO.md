...
- [ ] **`BENCHMARK_REPORT.md`** à la racine (§V.7) :
      ...
      - 1 étude d'ablation (avant/après un changement, mêmes tâches/modèle) — p. ex. impact de
        la validation forcée, ou des consignes `run_tests`/verbatim côté MBPP
      - Conclusions : modèles retenus / écartés, justifiés par les données
        (constat fort : `gpt-oss-*` incompatibles avec le code-based tool calling)
